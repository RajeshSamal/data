...
public class HKAGProcess {
	public static Map<String, String> importFields;

	static {
		importFields = new HashMap<String, String>();
		CustomObjectDao dao = ((CustomObjectDao) (DataInputProcessor
				.getDao(Constants.CUSTOM)));
		String fileType = Constants.HK_GOLD_ARCHIVE;
		String custObjectId = dao.getCustomObjectId(fileType);
		String languagePrefernceID = dao.getCustomFieldId(fileType,
				"LANGUAGE_PREFERENCE1");
		String clientID = dao.getCustomFieldId(fileType, "CLIENT_ID1");
		String genderId = dao.getCustomFieldId(fileType, "GENDER1");
		String pointBalanceID = dao.getCustomFieldId(fileType,
				"POINTS_BALANCE1");
		String vitalityStatusID = dao.getCustomFieldId(fileType,
				"VITALITY_STATUS1");
		String EntityReferenceID = dao.getCustomFieldId(fileType,
				"ENTITY_REFERENCE_NUMBER1");
		String aiaVitalityNumberID = dao.getCustomFieldId(fileType,
				"AIA_VITALITY_MEMBER_NUMBER1");
		String emailAddressID = dao
				.getCustomFieldId(fileType, "EMAIL_ADDRESS1");
		String memberSurNmaeID = dao.getCustomFieldId(fileType,
				"MEMBER_SURNAME1");
		String memberFirstNameID = dao.getCustomFieldId(fileType,
				"MEMBER_FIRST_NAMES1");
		...
		importFields.put("POINTS_BALANCE1", "{{CustomObject[" + custObjectId
				+ "].Field[" + pointBalanceID + "]}}");

		importFields.put("VITALITY_STATUS1", "{{CustomObject[" + custObjectId
				+ "].Field[" + vitalityStatusID + "]}}");

		importFields.put("ENTITY_REFERENCE_NUMBER1", "{{CustomObject["
				+ custObjectId + "].Field[" + EntityReferenceID + "]}}");

		importFields.put("AIA_VITALITY_MEMBER_NUMBER1", "{{CustomObject["
				+ custObjectId + "].Field[" + aiaVitalityNumberID + "]}}");

		importFields.put("EMAIL_ADDRESS1", "{{CustomObject[" + custObjectId
				+ "].Field[" + emailAddressID + "]}}");

		importFields.put("MEMBER_SURNAME1", "{{CustomObject[" + custObjectId
				+ "].Field[" + memberSurNmaeID + "]}}");
