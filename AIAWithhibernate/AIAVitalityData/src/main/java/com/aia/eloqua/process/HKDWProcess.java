...
public class HKDWProcess {
	...
	static {
		...
		String fileType= Constants.HK_DOWNGRADE_WARNING;
		...
		importFields.put("POINTS_TO_MAINTAIN_STATUS1", "{{CustomObject["
				+ custObjectId + "].Field[" + pointsToMantainId+ "]}}");
	...
	public static CDODetails process(CommonModel model) {
		...
		cdoData.setPointsToMantain(model.getPointsToMantainStatus());
	...
	private static Map<String, Object> prepareLeadData(CDODetails cdoDetail) {
		...
		if (cdoDetail != null) {
			...
			cdoData.put("POINTS_TO_MAINTAIN_STATUS1", cdoDetail.getPointsToMantain());
		}
		return cdoData;
	}
