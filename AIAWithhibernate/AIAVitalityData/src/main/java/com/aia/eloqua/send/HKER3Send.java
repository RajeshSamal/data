...
public class HKER3Send {
	...
	public static void sendDistinctDuplicateToElqua() {
		...
		try {
			...
			List<HKEngagementReminder3> objectList = DataInputProcessor.hker3DAO
			...
			for (int i = 0; i < list.size(); i++) {
				...
				HKER3 = list.get(i);
				CDODetails cdoData = HKER3Process.processHKER3(HKER3);
			...
			if (status == 0) {
				for (int i = 0; i < list.size(); i++) {
					...
					List<DataFile> fileList = DataInputProcessor.fileDAO.get(HKER3.getFileName());
					if(fileList.size()>0){
			...
			DataInputProcessor.hker3DAO.updateList(list, session);
	...
	public static void sendToElqua() {
		...
		try {
			...
			List<HKEngagementReminder3> objectList = DataInputProcessor.hker1DAO
			...
			DataInputProcessor.hker3DAO.updateList(objectList, session);
